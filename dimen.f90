...
!
   ...
   allocate(penalty(nx,ny,nz))      ! Fourier transform of the impurity external potential
...
! if(limp) then                            ! Things for impurities.
    allocate(qr(N_par,3)); qr = 0.d0       ! Array test particles
    allocate(qv(N_par,3)); qv = 0.d0       ! Array test particles
    allocate(Stor(N_par,3)); Stor = 0.d0   ! Array test particles
    allocate(pcr(N_par,3)); pcr = 0.d0     ! Array test particles
    allocate(pcv(N_par,3)); pcv = 0.d0     ! Array test particles
    allocate(rimp(N_par,3))       ! Array test particles
    allocate(vimp(N_par,3))       ! Array test particles
    allocate(aimp(N_par,3))       ! Array test particles
    allocate(rimpold(N_par,3,3))       ! Array test particles
    allocate(vimpold(N_par,3,3))       ! Array test particles
    allocate(aimpold(N_par,3,2))       ! Array test particles
    allocate(fx(nx,ny,nz))      ! Array with F_x
    allocate(fy(nx,ny,nz))      ! Array with F_z
    allocate(fz(nx,ny,nz))      ! Array with F_z
!    allocate(upotx(nx,ny,nz))      ! Mean field for the impurity
!    allocate(potx4(nx,ny,nz))      ! 
    allocate( psix(nx,ny,nz))      ! Sqrt of denx
    allocate(hpsix(nx,ny,nz))      ! Laplaciana de psix
