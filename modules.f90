...
real    (kind=8)              :: ekinqx           ! Kinetic energy (impurity)
...
integer (kind=8)              :: pfftfw_x    ! Pointer for FFT forward
integer (kind=8)              :: pfftfw_y    ! Pointer for FFT forward
integer (kind=8)              :: pfftfw_z    ! Pointer for FFT forward
...
integer (kind=8)              :: pfftbk_uimp   ! Pointer for FFT bakward
...
integer (kind=8)              :: pfftbk_x   ! Pointer for FFT bakward
integer (kind=8)              :: pfftbk_y   ! Pointer for FFT bakward
integer (kind=8)              :: pfftbk_z   ! Pointer for FFT bakward
...
!------------------------------------------------------------------
module impur
integer     (kind=4)               :: N_par=100000   ! Number of test particles
...
real     (kind=8), allocatable ::  rimp(:,:), vimp(:,:), aimp(:,:)      ! Coordinates of the impurity.
real     (kind=8), allocatable ::  rimpold(:,:,:), vimpold(:,:,:), aimpold(:,:,:)      ! Coordinates of the impurity.
logical                          :: limp=.false. ! T-> Impurity, F->Pure
real      (kind=8)               :: umax=400.d0  ! Maximum value for the potential
integer   (kind=4)               :: nq=1000      ! Number of q-values for Patil
...
real       (kind=8),allocatable   :: psix(:,:,:)  ! Sqrt of denx
real       (kind=8),allocatable   :: hpsix(:,:,:)     ! Laplaciana de psix
...
real    (kind=8) , parameter   :: mp_u = 0.0206149591055d0 ! 1 uma mass in Angs**-2 * Kelvin**-1, go figure!
